      ...
      <table class="commande-table">
        ...
        <tbody>
          <tr v-for="(user, index) in users" :key="user.id">
           <td>{{ index + 1}}</td>
            <td :class="{'connected-user': user.nom === authStore.utilisateur.nom}">
              {{ user.nom }}
            </td>
            <td>{{ user.email }}</td>
            <td>{{ user.role }}</td>
            <td class="text-center d-flex">
              ...
              <button
                v-if="authStore.utilisateur.role === 'ADMIN'"
              ...
              <!-- Edit and view buttons accessible by all roles -->
              <router-link :to="`/dashboard/utilisateurs/modifier/${user.id}`" class="btn btn-warning btn-sm me-2">
              ...
              <router-link :to="`/dashboard/utilisateurs/detail/${user.id}`" class="btn btn-info btn-sm me-2">
...
onMounted(async () => {
  try {
      ...
      users.value = userStore.users.sort((a, b) => a.id - b.id);
  } catch (error) {
