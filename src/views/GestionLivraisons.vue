<template>
  <div class="livraison-management">
    <div class="top-bar">
    ...
    <table class="livraison-table">
      ...
      <tbody>
          <tr v-for="(livraison, index) in filteredLivraisons" :key="index">
            <td>{{ livraison.id }}</td>
            <td>{{ livraison.nom }}</td>
          ...
          <td class="text-center">
            ...
            <router-link :to="`/dashboard/livraisons/modifier/${livraison.id}`" class="btn btn-warning btn-sm me-2">
            ...
            <button @click="viewLivraison(livraison)" class="btn btn-info btn-sm me-2">
...
onMounted(async() => {
  ...
  livraisons.value = livraisonStore.livraisons;
   }
   catch (error) {
      console.error("Erreur lors du chargement des livraisons:", error.message);
...
const confirmRemoveLivraison = async (id) => {
const result = await Swal.fire({
  ...
  if (result.isConfirmed) {
    try {
    await livraisonStore.removeLivraison(id);
